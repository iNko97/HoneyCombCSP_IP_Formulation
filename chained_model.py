...
def optimise(order_number, scenario_id, _n_s_max):
    ...
    order_filename = f"./Output/chained_model/order_{order_number}_{scenario_id}_{_n_s_max}.csv"
    results_filename = f"./Output/chained_model/results.csv"
    ...
    if _n_s_max == 2:
        if scenario_id == 1:
            ...
            load_ns_max = 2
        model.read(f"./Gurobi/chained_model/solution_{order_number}_{load_scenario_id}_{load_ns_max}.mst")
    ...
    if model.status == GRB.OPTIMAL or model.status == GRB.TIME_LIMIT or model.status == GRB.INTERRUPTED:
        ...
        model.write(f"./Gurobi/chained_model/solution_{order_number}_{scenario_id}_{_n_s_max}.mst")
